...
import { create, findByCondition, deleteById }  from "../dao/common.dao";
const { backOrderItems } = require("../dao/backOrder.dao");

const addbackOrderProducts = async (req, res, next) => {
  try {
    const { wholesalerInventoryId } = req.body;
    let backOrder = await findByCondition("backOrder", {
    ...
    if (backOrderItem) {
      const data = await findByCondition("backorder_item", {
        id: backOrderItem.id,
      });
      ...
      return handleSuccessResponse(
        res,
        data,
        "Back order item removed successfully"
      );
    }
    if (!backOrderItem) {
      const data = {
      ...
      return handleSuccessResponse(
        res,
        backOrderItem,
        "Back order item added successfully"
      );
    }
  } catch (error) {
    next(error);
