import { create, deleteById, bulkCreate }  from "../dao/common.dao";
import {
  handleSuccessResponse,
  handleErrorResponse,
  handleSuccessOrderResponse,
} from "../utility/handleResponse";
...
const getOrderWithFilter = async (req, res, next) => {
  try {
    ...
    if (status == "approved") {
      ...
      if (getApprovedItems.rows.length == 0) {
        throw ApiError.badRequest("No Orders");
      }
    ...
    if (status == "processed") {
      ...
      if (getProcessedItems.rows.length == 0) {
        throw ApiError.badRequest("No Orders");
      }
    ...
    if (status == "shipped") {
      ...
      if (getShippedItems.rows.length == 0) {
        throw ApiError.badRequest("No Orders");
      }
      const shippedOrders = [];
    ...
    if (status == "returns") {
      ...
      if (getReturnItems.rows.length == 0) {
        throw ApiError.badRequest("No Orders");
      }
      const returnOrders = [];
    ...
    if (status == "Delivered") {
      ...
      if (getDeliveredItems.rows.length == 0) {
        throw ApiError.badRequest("No Orders");
      }
    ...
    if (status == "Cancelled") {
      return handleSuccessOrderResponse(res, 0, [], "No Orders");
    }
  } catch (error) {
    next(error);
  }
